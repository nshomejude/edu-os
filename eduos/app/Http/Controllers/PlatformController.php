...
class PlatformController extends Controller
{
    ...
    public function saveSettings(Request $request)
    {
        $data = $request->validate([
            ...
            'exception_sla_low_hours' => 'nullable|integer|min:1|max:2000',
            'exception_sla_medium_hours' => 'nullable|integer|min:1|max:2000',
            'exception_sla_high_hours' => 'nullable|integer|min:1|max:2000',
            'exception_sla_critical_hours' => 'nullable|integer|min:1|max:2000',
        ]);
        ...
        foreach (['low', 'medium', 'high', 'critical'] as $sev) {
            if (! empty($data["exception_sla_{$sev}_hours"])) {
                \App\Modules\Platform\Models\Setting::put("exception_sla_{$sev}_hours", (string) $data["exception_sla_{$sev}_hours"]);
            }
        }
