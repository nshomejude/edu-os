...
@section('content')
    ...
    <div class="card mb">
        <form class="toolbar" method="get" style="margin:0">
            ...
            <button class="btn btn-secondary btn-sm">{{ __('Apply') }}</button>
            <a class="btn btn-sm btn-secondary" href="{{ route('reports.audit.csv', request()->query()) }}">{{ __('Export CSV') }}</a>
            @if ($q || request('from') || request('to'))
                <a class="btn btn-sm btn-secondary" href="{{ route('audit.index') }}">{{ __('Clear') }}</a>
            @endif
        </form>
    </div>
    ...
    <div class="card">
        <table class="table">
            <thead><tr><th>{{ __('When') }}</th><th>{{ __('Stream') }}</th><th>{{ __('Event') }}</th><th>{{ __('Actor') }}</th><th>{{ __('Chain') }}</th></tr></thead>
            <tbody>
            @forelse ($events as $e)
                <tr>
