...
<div class="login-wrap"><div class="login-card">
    ...
    <h1 style="text-align:center;font-size:22px;margin-bottom:4px;">{{ __('Forgot password') }}</h1>
    ...
    <form method="post" action="{{ route('password.email') }}">@csrf
        <div class="field" style="margin-bottom:18px"><label>{{ __('Email') }}</label><input class="input" type="email" name="email" required autofocus></div>
        <button class="btn btn-primary" style="width:100%;justify-content:center;">{{ __('Send reset link') }}</button>
    </form>
