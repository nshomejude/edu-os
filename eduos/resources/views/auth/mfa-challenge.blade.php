...
<div class="login-wrap"><div class="login-card" style="text-align:center">
    ...
    <h1 style="font-size:22px;margin-bottom:4px;">{{ __('Two-factor code') }}</h1>
    <p style="color:var(--text-2);font-size:13.5px;margin-bottom:18px;">Enter the 6-digit code from your authenticator app, or one of your one-time recovery codes.</p>
    @if (session('flash_error'))<div class="flash error">{{ session('flash_error') }}</div>@endif
    <form method="post" action="{{ route('mfa.verify') }}">@csrf
        <input class="input" name="code" maxlength="20" required autofocus
               style="text-align:center;font-size:24px;letter-spacing:8px;margin-bottom:18px">
        <button class="btn btn-primary" style="width:100%;justify-content:center;">{{ __('Verify') }}</button>
    </form>
</div></div>
</body></html>
